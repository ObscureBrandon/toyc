...
import { apiClient, type Token, type LexerResponse, type ParserResponse } from '@/lib/api';

type ViewMode = 'lexer' | 'parser';

export default function Home() {
  const placeholderCode = `x = 5 + 3.14 * (y - 2)
result = x / 2`;
  
  const [viewMode, setViewMode] = useState<ViewMode>('lexer');
  const [sourceCode, setSourceCode] = useState(placeholderCode);
  const [lexerResult, setLexerResult] = useState<LexerResponse | null>(null);
  const [parserResult, setParserResult] = useState<ParserResponse | null>(null);
  ...
  const handleParse = async () => {
    const codeToParse = sourceCode.trim() || placeholderCode;
    
    setLoading(true);
    setError(null);
    
    try {
      const response = await apiClient.parseCode(codeToParse);
  ...
  const getTokenColor = (type: string): string => {
    const colors: Record<string, string> = {
      ...
      'EQUAL': 'bg-yellow-900/20 text-yellow-300 border border-yellow-700/30',
      'LPAREN': 'bg-gray-800/50 text-gray-300 border border-gray-600/30',
      'RPAREN': 'bg-gray-800/50 text-gray-300 border border-gray-600/30',
      'ILLEGAL': 'bg-red-900/20 text-red-300 border border-red-700/30',
      'EOF': 'bg-gray-800/30 text-gray-500 border border-gray-600/20'
    };
    return colors[type] || 'bg-gray-800/50 text-gray-300 border border-gray-600/30';
  };

  return (
    <div className="min-h-screen bg-gray-900 text-gray-100">
      <div className="max-w-7xl mx-auto p-6">
        <h1 className="text-4xl font-bold mb-8 text-center text-white">ToyC Compiler Visualizer</h1>
        ...
        <div className="mb-8 flex justify-center">
          <div className="bg-gray-800 p-1 rounded-lg border border-gray-600">
            <div className="flex items-center gap-1">
              ...
              <Link 
                href="/visualize"
                className="px-6 py-2 bg-green-600 text-white rounded-md font-medium transition-colors hover:bg-green-700 flex items-center gap-2"
              >
                <Play size={16} />
                Step-by-Step
              </Link>
        ...
        <div className="mb-6 flex gap-4 justify-center">
          {viewMode === 'lexer' ? (
            ...
            <button
              onClick={handleParse}
              disabled={loading}
              className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-gray-600 disabled:cursor-not-allowed font-medium transition-colors"
            >
        ...
        {/* Results */}
        {viewMode === 'lexer' && lexerResult && (
          <div className="space-y-8">
            <div>
              <h2 className="text-2xl font-semibold mb-4 text-gray-200">Token Visualization</h2>
              <div className="flex flex-wrap gap-3">
                {lexerResult.tokens.map((token, idx) => (
                  token.type !== 'EOF' && (
                    <div
                      key={idx}
                      className={`px-4 py-3 rounded-lg shadow-sm ${getTokenColor(token.type)}`}
                    >
                      <div className="text-xs font-bold uppercase tracking-wide">{token.type}</div>
            ...
            <div>
              <h2 className="text-2xl font-semibold mb-4 text-gray-200">Token Stream</h2>
              <div className="bg-gray-800/50 rounded-lg overflow-hidden shadow-sm border border-gray-700/30">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-700/50">
                      <tr>
                        <th className="text-left py-3 px-4 font-semibold text-gray-300">Position</th>
                        <th className="text-left py-3 px-4 font-semibold text-gray-300">Type</th>
                        <th className="text-left py-3 px-4 font-semibold text-gray-300">Literal</th>
                      </tr>
                    </thead>
                    <tbody>
                      {lexerResult.tokens.map((token, idx) => (
                        <tr key={idx} className="border-b border-gray-700/30 hover:bg-gray-700/20">
                          <td className="py-3 px-4 font-mono text-gray-400">{token.position}</td>
                          <td className="py-3 px-4 font-mono font-medium text-gray-200">{token.type}</td>
        ...
        {viewMode === 'parser' && (
          <div>
            <h2 className="text-2xl font-semibold mb-4 text-gray-200">Abstract Syntax Tree</h2>
            <div className="h-96">
              <ASTVisualizer 
                ast={parserResult?.success ? parserResult.ast : null} 
                error={parserResult?.success === false ? parserResult.error : undefined}
              />
            </div>
          </div>
        )}
